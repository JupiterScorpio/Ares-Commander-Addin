...
namespace ARES_Preval
{
    public class NBCrelate
    {
        ...
        //public static Rulecheckprogress frm = new Rulecheckprogress();
        ...
        {
            if (Commands.amroadrule.Count == 0)
            {
                getonlyMroadentity();
            }
            List<JsonItems> jsonlists = new List<JsonItems>();
            IEnumerable<winrule> windrulelst = Commands.awindowrule.Distinct();
            IEnumerable<doorrule> doorrulelst = Commands.adoorrule.Distinct();
            IEnumerable<roomrule> rmrulelst = Commands.aroomrule.Distinct();
            IEnumerable<mroadrule> mroadlst = Commands.amroadrule.Distinct();
            ...
            foreach (winrule wrule in windrulelst)
            {
                jsonlists.Add(new JsonItems()
                {
                    layer = "_MainRoad",
            ...
            if (Plugin.strCurJsonPath == "")
            {
                ...
                if (openFileDialog1.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    System.IO.File.WriteAllText(Plugin.strCurJsonPath, Json1);
                }
            }
            else
            {
                var Json = JsonConvert.SerializeObject(jsonlists);
                System.IO.File.WriteAllText(Plugin.strCurJsonPath, Json);
            }
        }
        public static void GettingEntities()
        {
            ...
            try
            {
                Rulecheckprogress frm = new Rulecheckprogress();
                frm.Show();
                foreach (string layername in Plugin.allLayers)
                {
                    ...
                    //System.IO.File.WriteAllText(@"D:\splayCompareCross.txt", layername);
        ...
        {
            var documentManager = Teigha.ApplicationServices.Application.DocumentManager;
            var currentDocument = documentManager.MdiActiveDocument;
                            ...
                            if (subent.Layer == "_MainRoad")
                            {
                                if (id.ObjectClass == PlineCls)
                                {
                                    ...
                                    if (pline.Layer.Equals("_MainRoad", System.StringComparison.CurrentCultureIgnoreCase))
                                    {
                                        ...
                                        mrule.hnd = pline.ObjectId.Handle.ToString();
                                        Commands.amroadrule.Add(mrule);
                                    }
                                }
                            }
                        }
                ...
                case "_Window":
                    {
                        ...
                        //            {
                        //                double warea = 0;
                        //                foreach (windowrule rule in Commands.awindowrule)
                        //                {
                ...
                case "_Door":
                    {
                        ...
                        //            {
                        //                double darea = 0;
                        //                foreach (doorrule rule in Commands.adoorrule)
                        //                {
                ...
                case "_MainRoad":
                    {
                        ...
                        {
                            string strmRdtxt = Plugin.amroadNmTxt[0].Contents.ToLower();
                ...
                case "_Room":
                    {
                        ...
                        {
                            bool bopen = false;
                            foreach (winrule wr in Commands.awindowrule)
                            {
                ...
                case "_ProposedWork":
                    {
                        ...
                        Editor ed = doc.Editor;
                        List<ObjectId> objidlist = new List<ObjectId>();
                        //Application.ShowAlertDialog(Plugin.aprpWrkTxt.Count.ToString()+":Text:"+Plugin.aprpwrkpline.Count.ToString());
                        foreach (Polyline plsply in Plugin.aprpwrkpline)
                        {
                            if (plsply.Closed)
                            {
        ...
        public static bool IsPointOnCurveGDAP(Curve3d cv, Point3d pt)
        {
            try
            {
                ...
                return cv.GetDistanceTo(pt) <= 0.2;
