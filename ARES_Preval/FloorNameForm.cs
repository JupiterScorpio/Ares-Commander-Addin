...
namespace ARES_Preval
{
    public partial class FloorNameForm : Form
    {
        ...
        private bool FloorNameFilter()
        {
            if (chk_typical.Checked && cmb_floorname.Text != "")
            {
                ...
                while (strtemp != "")
                {
                    ...
                    if (strsepSign == "-")
                    {
                        ...
                        {
                            Commands.floornamelst.Add(i.ToString());
            ...
            {
                Commands.tmpfloorsectionName = strfloorsectionname;
                Commands.floornamelst.Add(strfloorsectionname);
                Commands.tmpfloorName = strfloorname;
                Commands.bfloorTypical = false;
        ...
        public static void AssignFloorandFloorInSection()
        {
            ...
            var db = doc.Database;
            var ed = doc.Editor;
            Commands.SetLayerCurrent("_FloorInSection");
        ...
        FloorNameRpeat:
            ...
            using (DocumentLock docLock = doc.LockDocument())
            {
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    Commands.SetLayerCurrent("_Floor");
                    ...
                    if ((string)Application.GetSystemVariable("clayer") == "_Floor")
                    {
                        if (result1.Status == PromptStatus.OK)
                        {
                            ...
                            if (poly1 != null && poly1.Layer == "_Floor")
                            {
                                ...
                                txt.SetDatabaseDefaults(db);
                                Point3d ptleft = Commands.Getleft(poly1);
                                Point3d ptright = Commands.Getright(poly1);
                                Point3d pttop = Commands.Gettop(poly1);
                                Point3d ptbottom = Commands.Getbottom(poly1);
                                double width = ptright.X - ptleft.X;
                                double height = pttop.Y - ptbottom.Y;
                                txt.Height = height / 3; //<==change to your default height
                                txt.Width = width / 3;
